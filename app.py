...
def receive_text():
    ...
    get_geo_url = \
        'http://localfreeweb.cartodb.com/api/v2/sql?q=SELECT stop_lat, stop_lon FROM stops WHERE stop_id = '
    get_geo_url += stop_id
    ...
    get_closest_free_net_url = \
        'http://localfreeweb.cartodb.com/api/v2/sql?q=SELECT name, address, zip, phone, ST_Distance(the_geom::geography, ST_PointFromText(\'POINT(' \
        + geo_long + ' ' + geo_lat \
        + ')\', 4326)::geography) AS distance FROM freeweb ORDER BY distance ASC LIMIT 3'
